    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if compile:
                    compiled_transformer = torch.compile(self.transformer, mode='max-autotune', fullgraph=True, dynamic=True)
                else:
                    compiled_transformer = self.transformer
                if i > 0:
                    norms.append(torch.sqrt(torch.sum((noise_pred - noise_pred_old) ** 2)))
        ...
        return StableDiffusion3PipelineOutput(images=image), norms
